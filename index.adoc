...
https://github.com/Timmmm/fst_spec[Source code for this document]

== Introduction

WARNING: This is an incomplete draft! Also I didn’t create the FST format so there may be errors in this specification. If you find any please https://github.com/Timmmm/fst_spec/issues/new[let me know].
...
TIP: https://github.com/johnjohnlin[Yu-Sheng Lin] has generously donated *https://github.com/Timmmm/fst_spec/tree/master/sample_code[sample Python code]* based on this spec.
...
| [[header_num_hierarchy_vars,`header_num_hierarchy_vars`]]`header_num_hierarchy_vars` | 49     | `u64`     | Number of variables in the hierarchy.
...
| [[hierarchy_compressed_once_length,`hierarchy_compressed_once_length`]]`hierarchy_compressed_once_length` | 17      | `varint` | _Only present for <<FST_BL_HIER_LZ4DUO>>._ Length of the data after it has been compressed once.
| [[hierarchy_data,`hierarchy_data`]]`hierarchy_data`                                                       | depends | -        | Compressed hierarchy data.
|===
...
The field `hierarchy_compressed_once_length` is only present if the block type is `FST_BL_HIER_LZ4DUO`. It records the length of the data after one application of LZ4.
...
If the variable is *not* a 1-bit value then the `time_index_delta` is encoded as its own `varint` together with an encoding mode bit:
    ...
    time_index_delta << 1 | non_binary

If `non_binary` is 1 then this means the value contain values other than 0 and 1 (e.g. X, Z).
If `non_binary` is 0 then the value only contains 0's and 1's.
In this case the values are encoded as raw bits packed into a whole number of bytes.
The bits are stored in each byte in Big Endian order starting at the MSB. Below is an example of the sequence 0b00, 0b01, 0b10, 0b11, with `time_index_delta` of 1:
...
If `non_binary` is 1 then the data that follows is encoded as raw ASCII, e.g. "01Z011XX1".

The rules for `FST_VT_VCD_REAL` are slightly different:

* If `non_binary` is 0 then the bits of the `f64` are encoded as ASCII as before (this is unlikely to happen but it is possible). If they're 1 then it is a _native Endian_ `f64`.
